...
import { fetchHistoricalWeather, celsiusToFahrenheit, WeatherData } from './lib/weatherApi';
import { predictNextTemperature, preprocessData, calculateMetrics } from './lib/prediction';
...
export default function App() {
  ...
  const handleLocationSelect = async (lat: number, lon: number, locationName: string) => {
    ...
    try {
      ...
      const weatherResponse = await fetchHistoricalWeather(lat, lon, 14);
      
      // Preprocess data
      const processedData = preprocessData(weatherResponse.historicalData);
      ...
      // Run prediction
      const prediction = predictNextTemperature(processedData);
    ...
    <div className="h-screen w-full flex flex-col bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      <footer className="bg-white border-t border-gray-200 px-6 py-4 shadow-lg">
        <p className="text-xs text-gray-500 text-center">
          ...
          Predictions use ensemble time-series methods (EMA, Linear Regression, Trend Analysis)
        </p>
      </footer>
    </div>
  );
